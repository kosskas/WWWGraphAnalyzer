...
class WWWGraph:
    ...
    def extract_subgraph(self, graph, node):
        # N and N
    ...
    def print_linregress(self, x, y, label):
        ...
        b = slope
        a = np.exp(intercept)
        print(f"[{self.label}]  Wzór rozkładu potęgowego ({label}):")
        print(f"[{self.label}] P(k) = {a:.4f} * x^{b:.4f}")
        print(f"[{self.label}] R^2 = {r_value**2:.2f}\n")
    ...
    def distances(self, graph):
        ...
        a, b, c = coeffs
        print(
            f"[{self.label}]  Regresja kwadratowa: {a:.4f} * x2 + {b:.4f} * x + {c:.4f}"
        )
    ...
    def vertex_connectivityNP(self, graph):
        try:
            ...
            k = 1  # nx.node_connectivity(graph)
            print(f"[OK] Vertex connectivity (node_connectivity) = {k}")
            cuts = 0
            if k == 1:
                cuts = sum(1 for _ in nx.all_node_cuts(undirected_G))
                print(f"[{self.label}] k=1, cuts {cuts}")
            elif k == 2:
                pairs = len(list(nx.k_edge_components(graph, k)))
                print(f"[{self.label}] k=2, cuts {pairs}")
            else:
                print(k)
    ...
    def vertex_connectivity(self, graph):
        ...
        pairs = list(nx.all_node_cuts(self.f, 2))
        print(f"slabe {pairs}")
        # print(f"[OK] Vertex connectivity (node_connectivity) = {k}")

    def pagerank1(self, graph):
    ...
    def custom_pagerank(self, graph, alpha):
        ...
        x_vals = list(vertex_to_id.values())
        y_vals = list(sorted_pagerank.values())
        self.print_linregress(x_vals, y_vals, f"[{self.label}] pg a={alpha}")
        self.save_file(
            f"pg a={alpha}_{self.label}",
    ...
    def simulate_failure_and_attack_scenarios(self, Graph, idx, seed=None):
        ...
        attack_30 = [node for node, _ in degree_sorted[: int(0.30 * n)]]
        ...
        if 8 == idx:
            scenarios = remove_nodes(Graph, attack_30)
        if 8 == idx:
